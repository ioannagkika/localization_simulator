...
class App(customtkinter.CTk):
    ...
    def add_marker_event(self, coords = (0,0)):
        ...
        if (len(self.new_marker_1)>=3) and (self.new_marker_1[-2][0] != 0) and (self.new_marker_1[-1][0] != 0):
            ...
            if sum(self.l) > float(self.speed.get())*float(self.time.get()):
                self.l = []
            ...
            if sum(self.l) == float(self.speed.get())*float(self.time.get()):
                ...
                self.P1.append(self.new_marker_1[-1])
                self.d = 0
            while sum(self.l) > float(self.speed.get())*float(self.time.get()):
                endiameso = destination(lat2 = self.new_marker_1[-1][0], long2 = self.new_marker_1[-1][1],
                                    ...
                                    kms = float(self.speed.get())*float(self.time.get()) - sum(self.l[:-1]))
                ...
                self.l = [sum(self.l) - float(self.speed.get())*float(self.time.get())]
